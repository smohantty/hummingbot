...
class BithumbExchange(ExchangePyBase):
    ...
    def __init__(self,
        ...
        self.secret_key = bithumb_api_secret
        self._domain = domain
    ...
    @staticmethod
    def bithumb_order_type(order_type: OrderType) -> str:
        return order_type.name.upper()

    @staticmethod
    def to_hb_order_type(bithumb_type: str) -> OrderType:
        return OrderType[bithumb_type]
    ...
    async def _place_order(self,
        ...
        type_str = BithumbExchange.bithumb_order_type(order_type)
    ...
    async def _user_stream_event_listener(self):
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                # Refer to https://github.com/bithumb-exchange/bithumb-official-api-docs/blob/master/user-data-stream.md
                # As per the order update section in Bithumb the ID of the order being canceled is under the "C" key
    ...
    async def _update_order_fills_from_trades(self):
        ...
        in case Bithumb's user stream events are not working.
        NOTE: It is not required to copy this functionality in other connectors.
        This is separated from _update_order_status which only updates the order status without producing filled
        events, since Bithumb's get order endpoint does not return trade IDs.
        ...
        if (long_interval_current_tick > long_interval_last_tick
            ...
            query_time = int(self._last_trades_poll_bithumb_timestamp * 1e3)
            self._last_trades_poll_bithumb_timestamp = self._time_synchronizer.time()
